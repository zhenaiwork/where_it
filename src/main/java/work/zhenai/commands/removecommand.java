...
public class removecommand {
    public static void remove(CommandDispatcher<ServerCommandSource> dispatcher){
        dispatcher.register(literal("remove").requires(c -> c.hasPermissionLevel(4)).then(literal("last").executes(c -> config.remove(c.getSource().getPlayer()))));
    ...
    private static int help(PlayerEntity player){
        player.sendMessage(new LiteralText("remove last 删除最后一个标记"), false);
        return 0;
    }
}
